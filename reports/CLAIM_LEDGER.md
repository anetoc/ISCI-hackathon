...
|---|-------|--------|----------|-------------|
| 1 | Among detectable-effect perturbations, axis-specificity + cross-donor coherence identify known T-cell-state regulators **beyond effect magnitude** | **PASS** | Authoritative pre-specified M→M+C: +0.357 AUPRC (0.539→0.896), 95% CI [+0.117,+0.538], P>0=1.000. Leakage-free OOF: +0.215 [+0.074,+0.560], permutation p=0.010. Descriptive ranking: 0.415→0.722; matched three-condition C-vs-M aggregate: +0.229 [0.072,0.405]. Orthogonal to magnitude ρ=+0.02; replicated across Rest/Stim8/Stim48 | Estimands are distinct; not a claim about every gene — restricted to detectable-effect set and canonical axis-defining regulators |
...
| 17 | The GSE190604 no-stim versus stimulated Th2 difference establishes a context interaction | **DIRECTIONAL-UNCERTAIN post-result** | Target-paired contrast +0.190, paired bootstrap [+0.046,+0.370] with 99.8% positive, but within-gene context-swap p=0.091 (1,000 swaps). GEO documents two paired donors mixed 1:1 before droplet loading; processed matrix lacks donor identity. Raw recovery is a confirmatory no-go: ~154.2 GB mRNA download for only two donors | Stable to target resampling is not significance under context exchange; donor-mixed is not donor-resolved and not a stimulation mechanism |
