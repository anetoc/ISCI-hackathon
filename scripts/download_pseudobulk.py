...
KEY = "marson2025_data/GWCD4i.pseudobulk_merged.h5ad"
OUT = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "data", "marson",
    ...
    with urllib.request.urlopen(req, timeout=120) as r, open(OUT, "ab") as f:
        while True:
            ...
            if now - last > 30:
                ...
                eta = int((total - got) / max(rate * 1e6, 1))
                print(f"  {got/1e9:6.2f}/{total/1e9:.2f} GB  {100*got/total:5.1f}%  "
                      f"{rate:5.1f} MB/s  eta {eta}s", flush=True)
                last = now
